...
public class IslandRule implements Rule {
	...
	private void shapeMap(Map theMap, String[] args) {
		...
		int mapSize = theMap.getLengthX() * theMap.getLengthY() - theMap.getPerimeterLength();

		System.out.println(waterTileCount+"/"+ mapSize +"="+(waterTileCount / mapSize));
		...
		while (waterCoverage < waterPercentage){
			...
			System.out.println(waterTileCount+"/"+ mapSize +"="+waterCoverage);
			System.out.println(waterCoverage+"<"+ waterPercentage + ":" + (waterCoverage < waterPercentage ));
			int modifiedEdge = RandomGenerator.getInstance().nextInt(TOTAL_EDGE);
			...
			if (modifiedEdge == NORTH_EDGE || modifiedEdge == SOUTH_EDGE) {
				...
				if (modifiedEdge == NORTH_EDGE) {
					j = 1;
					System.out.println("NorthSide\nStarting coords:" + modifiedTileCoord + "," + j);
					while (theMap.getTile(modifiedTileCoord, j).getSaturation() > 0) {
						j++;
					}
					System.out.println("Final coords:" + modifiedTileCoord + "," + j);
				}
				...
				if (modifiedEdge == EAST_EDGE) {
					i = 1;
					System.out.println("EastSide\nStarting coords:" + modifiedTileCoord + "," + i);
					while (theMap.getTile(i, modifiedTileCoord).getSaturation() > 0) {
						i++;
					}
					System.out.println("Final coords:" + i + "," + modifiedTileCoord);
				}
				// else its the south edge, so set j to the first possible south
				// edge and decrement
				else {
